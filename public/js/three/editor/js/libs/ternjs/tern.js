...
(function(root, mod) {
        ...
        if (init && init.passes)
          for (var type in init.passes)
            if (init.passes.hasOwnProperty(type))
              (this.passes[type] || (this.passes[type] = [])).push(
                init.passes[type],
              );
      }
  ...
  function findCompletions(srv, query, file) {
    ...
    var wrapAsObjs =
      query.types || query.depths || query.docs || query.urls || query.origins;
    ...
    if (exprAt) {
      if (
        exprAt.node.type == 'MemberExpression' &&
        exprAt.node.object.end < wordStart
      ) {
        memberExpr = exprAt;
