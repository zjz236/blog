...
(function(mod) {
        ...
        CallExpression: function(node, scope) {
          if (node.commentsBefore && isDefinePropertyCall(node)) {
            ...
            if (type && type instanceof infer.Obj) {
              ...
              if (prop)
                interpretComments(node, node.commentsBefore, scope, prop);
            }
          }
        },
      },
  ...
  function interpretComments(node, comments, scope, aval, type) {
    ...
    if (!type && aval instanceof infer.AVal && aval.types.length) {
      ...
      if (
        !(type instanceof infer.Obj) ||
        type.origin != cx.curOrigin ||
        type.doc
      )
        type = null;
    }
  ...
  function parseTypeInner(scope, str, pos) {
    ...
    if (str.indexOf('function(', pos) == pos) {
      ...
      if (/^(number|integer)$/i.test(word)) type = infer.cx().num;
        ...
        while (
          str.charCodeAt(pos) == 46 ||
          acorn.isIdentifierChar(str.charCodeAt(pos))
        )
          ++pos;
  ...
  function propagateWithWeight(type, target) {
    ...
    type.type.propagate(
      target,
      weight || (type.madeUp ? WG_MADEUP : undefined),
    );
  }
  ...
  function applyType(type, self, args, ret, node, aval) {
    ...
    if (node.type == 'VariableDeclaration') {
      ...
      if (node.value.type == 'FunctionExpression')
        fn = node.value.body.scope.fnType;
    }
