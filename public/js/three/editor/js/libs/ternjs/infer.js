...
(function(root, mod) {
  ...
  AVal.prototype = extend(ANull, {
    ...
    computedPropType: function() {
      ...
      if (this.propertyOf.hasProp('<i>')) {
        ...
        this.propertyOf.maybeProps &&
        this.propertyOf.maybeProps['<i>'] == this
      ) {
  ...
  Obj.prototype = extend(Type.prototype, {
    ...
    getProp: function(prop) {
      var found =
        this.hasProp(prop, true) || (this.maybeProps && this.maybeProps[prop]);
  ...
  Arr.prototype = extend(Obj.prototype, {
    ...
    toString: function(maxDepth) {
      ...
      return (
        '[' +
        (maxDepth > -3
          ? toString(this.getProp('<i>'), maxDepth - 1, this)
          : '?') +
        ']'
      );
    },
  ...
  function maybeInstantiate(scope, score) {
    if (scope.fnType)
      scope.fnType.instantiateScore =
        (scope.fnType.instantiateScore || 0) + score;
  }
  ...
  exports.typeFromContext = function(ast, found) {
    ...
    if (findTypeFromContext.hasOwnProperty(parent.type)) {
      type = findTypeFromContext[parent.type](parent, found.node, function(
        node,
        fromContext,
      ) {
    ...
    walk.simple(
      ast,
      {
        MemberExpression: function(node, scope) {
          ...
          if (findType(node.object, scope).getType() == objType)
            f(node.property);
        },
        ObjectExpression: function(node, scope) {
          if (findType(node, scope).getType() != objType) return;
          for (var i = 0; i < node.properties.length; ++i)
            if (node.properties[i].key.name == propName)
              f(node.properties[i].key);
        },
      },
      simpleWalker,
      scope,
