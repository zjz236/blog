...
THREE.ConvexObjectBreaker.prototype = {
  ...
  prepareBreakableObject: function(
    object,
    mass,
    velocity,
    angularVelocity,
  ...
  subdivideByImpact: function(
    ...
    function subdivideRadial(subObject, startAngle, endAngle, numIterations) {
      if (
        Math.random() < numIterations * 0.05 ||
        numIterations > maxTotalIterations
      ) {
      ...
      if (numIterations === 0) {
        ...
        if (numIterations <= maxRadialIterations) {
          angle =
            (endAngle - startAngle) * (0.2 + 0.6 * Math.random()) + startAngle;
          ...
          angle =
            (0.5 * (numIterations & 1) + 0.2 * (2 - Math.random())) * Math.PI;
  ...
  cutByPlane: function(object, plane, output) {
    ...
    if (numPoints1 > 4) {
      object1 = new THREE.Mesh(
        new THREE.ConvexBufferGeometry(points1),
        object.material,
      );
    ...
    if (numPoints2 > 4) {
      object2 = new THREE.Mesh(
        new THREE.ConvexBufferGeometry(points2),
        object.material,
      );
...
THREE.ConvexObjectBreaker.transformPlaneToLocalSpace = (function() {
  ...
  return function transformPlaneToLocalSpace(plane, m, resultPlane) {
    ...
    var referencePoint = THREE.ConvexObjectBreaker.transformTiedVectorInverse(
      plane.coplanarPoint(v1),
      m,
    );
