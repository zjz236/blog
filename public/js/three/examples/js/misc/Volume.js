...
THREE.Volume.prototype = {
  ...
  extractPerpendicularPlane: function(axis, RASIndex) {
    ...
    switch (axis) {
      case 'x':
        ...
        planeMatrix.setPosition(
          new THREE.Vector3(RASIndex - positionOffset, 0, 0),
        );
        break;
      case 'y':
        axisInIJK.set(0, 1, 0);
        ...
        planeMatrix.setPosition(
          new THREE.Vector3(0, RASIndex - positionOffset, 0),
        );
        break;
      case 'z':
      default:
        ...
        planeMatrix.setPosition(
          new THREE.Vector3(0, 0, RASIndex - positionOffset),
        );
        break;
    }
    ...
    var iDirection = [firstDirection, secondDirection, axisInIJK].find(function(
      x,
    ) {
      return Math.abs(x.dot(base[0])) > 0.9;
    });
    var jDirection = [firstDirection, secondDirection, axisInIJK].find(function(
      x,
    ) {
      return Math.abs(x.dot(base[1])) > 0.9;
    });
    var kDirection = [firstDirection, secondDirection, axisInIJK].find(function(
      x,
    ) {
    ...
    sliceAccess = function(i, j) {
      ...
      var si =
        iDirection === axisInIJK ? IJKIndex : iDirection.argVar === 'i' ? i : j;
      var sj =
        jDirection === axisInIJK ? IJKIndex : jDirection.argVar === 'i' ? i : j;
      var sk =
        kDirection === axisInIJK ? IJKIndex : kDirection.argVar === 'i' ? i : j;
