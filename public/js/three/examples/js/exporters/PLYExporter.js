  ...
  parse: function(object, onDone, options) {
    ...
    object.traverse(function(child) {
      if (child.isMesh === true) {
        ...
        if (geometry.isGeometry === true) {
          var bufferGeometry =
            geomToBufferGeom.get(geometry) ||
            new THREE.BufferGeometry().setFromObject(mesh);
    ...
    includeNormals =
      includeNormals && excludeAttributes.indexOf('normal') === -1;
    ...
    if (options.binary === true) {
      ...
      var vertexListLength =
        vertexCount *
        (4 * 3 +
          (includeNormals ? 4 * 3 : 0) +
          (includeColors ? 3 : 0) +
          (includeUVs ? 4 * 2 : 0));

      // 1 byte shape desciptor
      // 3 vertex indices at ${indexByteCount} bytes
      var faceListLength = includeIndices
        ? faceCount * (indexByteCount * 3 + 1)
        : 0;
      var output = new DataView(
        ...
        if (includeIndices === true) {
          ...
          if (indices !== null) {
            for (var i = 0, l = indices.count; i < l; i += 3) {
              ...
              output.setUint32(
                fOffset,
                indices.getX(i + 0) + writtenVertices,
                options.littleEndian,
              );
              ...
              output.setUint32(
                fOffset,
                indices.getX(i + 1) + writtenVertices,
                options.littleEndian,
              );
              fOffset += indexByteCount;
              ...
              output.setUint32(
                fOffset,
                indices.getX(i + 2) + writtenVertices,
                options.littleEndian,
              );
              fOffset += indexByteCount;
            }
      ...
      traverseMeshes(function(mesh, geometry) {
        ...
        if (includeIndices === true) {
          if (indices !== null) {
            ...
            for (var i = 0, l = vertices.count; i < l; i += 3) {
              faceList += `3 ${writtenVertices + i} ${writtenVertices +
                i +
                1} ${writtenVertices + i + 2}\n`;
            }
          }
