    ...
    createClass(XAnimationObj, [
      {
        ...
        value: function make(XAnimationInfoArray) {
          ...
          this.length = this.hierarchy[0].keys[
            this.hierarchy[0].keys.length - 1
          ].time;
        },
      },
      {
          ...
          for (var i = 0; i < XAnimationInfo.keyFrames.length; i++) {
            ...
            if (XAnimationInfo.keyFrames[i].matrix) {
              ...
              if (this.putFlags.putPos) {
                keyframe.pos = new THREE.Vector3().setFromMatrixPosition(
                  keyframe.matrix,
                );
              }

              if (this.putFlags.putRot) {
                keyframe.rot = new THREE.Quaternion().setFromRotationMatrix(
                  keyframe.matrix,
                );
              }

              if (this.putFlags.putScl) {
                keyframe.scl = new THREE.Vector3().setFromMatrixScale(
                  keyframe.matrix,
                );
              }
            }
              ...
              switch (this._currentObject.type) {
                case 'Mesh':
                  switch (mode) {
                    case 0:
                      this._readVertex1(
                        this._currentObject.data.substr(
                          endRead,
                          find - endRead,
                        ),
                      );
        ...
        value: function _makePutBoneList(_RootName, _bones) {
          ...
          for (var frame in this.HieStack) {
            if (this.HieStack[frame].name === _RootName || putting) {
              ...
              b.matrixWorld = b.matrix;
              b.FrameTransformMatrix = this.HieStack[
                frame
            ...
            for (var bi = 0; bi < this._currentGeo.BoneInfs.length; bi++) {
              ...
              for (
                ...
                switch (this._currentGeo.VertexSetedBoneCount[nowVertexID]) {
                  case 0:
                    this._currentGeo.GeometryData.skinIndices[
                      stride
                    ] = boneIndex;
                  ...
                  case 1:
                    ...
                    this._currentGeo.GeometryData.skinWeights[
                      stride + 1
                    ] = nowVal;
                    break;
                  case 2:
      ...
      },
      {
        ...
        value: function assignAnimation(_model, _animation) {
          ...
          model.geometry.animations.push(
            THREE.AnimationClip.parseAnimation(put, model.skeleton.bones),
          );
