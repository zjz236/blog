  ...
  while (true) {
    if (right - left <= 25) {
      for (j = left + 1; j <= right; j++) {
        ...
        while (
          i >= left &&
          arr[i * eleSize + orderElement] > swap[orderElement]
        ) {
          for (x = 0; x < eleSize; x++) {
      ...
      if (
        arr[left * eleSize + orderElement] > arr[right * eleSize + orderElement]
      ) {
        swapF(left, right);
      }

      if (
        arr[i * eleSize + orderElement] > arr[right * eleSize + orderElement]
      ) {
        swapF(i, right);
      }

      if (
        arr[left * eleSize + orderElement] > arr[i * eleSize + orderElement]
      ) {
        swapF(left, i);
      }
...
THREE.TypedArrayUtils.Kdtree = function(points, metric, eleSize) {
  ...
  function buildTree(points, depth, parent, pos) {
    ...
    node = new scope.Node(
      getPointSet(points, median),
      depth,
      parent,
      median + pos,
    );
    node.left = buildTree(
      points.subarray(0, median * eleSize),
      depth + 1,
      node,
      pos,
    );
    node.right = buildTree(
      points.subarray((median + 1) * eleSize, points.length),
      depth + 1,
      node,
      pos + median + 1,
    );
 ...
 */
THREE.TypedArrayUtils.Kdtree.prototype.Node = function(
  obj,
  depth,
  parent,
  pos,
