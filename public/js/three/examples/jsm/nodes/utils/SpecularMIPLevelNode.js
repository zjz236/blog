...
SpecularMIPLevelNode.prototype.generate = function(builder, output) {
  if (builder.isShader('fragment')) {
    ...
    var getSpecularMIPLevel = builder.include(
      SpecularMIPLevelNode.Nodes.getSpecularMIPLevel,
    );

    return builder.format(
      getSpecularMIPLevel +
        '( ' +
        this.roughness.build(builder, 'f') +
        ', ' +
        this.maxMIPLevel.build(builder, 'f') +
        ' )',
      this.type,
      output,
    );
    ...
    console.warn(
      'THREE.SpecularMIPLevelNode is not compatible with ' +
        builder.shader +
        ' shader.',
    );
