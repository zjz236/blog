...
ReflectorNode.prototype.setMirror = function(mirror) {
  ...
  this.textureMatrix = new Matrix4Node(
    this.mirror.material.uniforms.textureMatrix.value,
  );

  this.localPosition = new PositionNode(PositionNode.LOCAL);

  this.uv = new OperatorNode(
    this.textureMatrix,
    this.localPosition,
    OperatorNode.MUL,
  );
  ...
  this.texture = new TextureNode(
    this.mirror.material.uniforms.tDiffuse.value,
    this.uv,
    null,
    true,
  );
...
ReflectorNode.prototype.generate = function(builder, output) {
  if (builder.isShader('fragment')) {
    ...
    return builder.format(
      this.texture.build(builder, this.type),
      this.type,
      output,
    );
  } else {
    console.warn(
      'THREE.ReflectorNode is not compatible with ' +
        builder.shader +
        ' shader.',
    );
