...
StandardNode.prototype.build = function(builder) {
  ...
  var useClearcoat =
    this.clearcoat || this.clearcoatRoughness || this.clearCoatNormal;
  ...
  if (builder.isShader('vertex')) {
    ...
    var specularRoughness = new ExpressionNode(
      'material.specularRoughness',
      'f',
    );
    var clearcoatRoughness = new ExpressionNode(
    ...
    var contextEnvironment = {
      ...
      worldNormal: new ExpressionNode(
    ...
    var contextClearcoatEnvironment = {
      ...
      worldNormal: new ExpressionNode(
        'inverseTransformDirection( geometry.clearcoatNormal, viewMatrix )',
        'v3',
      ),
      gamma: true,
    };
    ...
    var clearcoat = this.clearcoat
      ? this.clearcoat.flow(builder, 'f')
      : undefined;
    var clearcoatRoughness = this.clearcoatRoughness
      ? this.clearcoatRoughness.flow(builder, 'f')
      : undefined;
    var clearcoatNormal = this.clearcoatNormal
      ? this.clearcoatNormal.flow(builder, 'v3')
      : undefined;
    ...
    var light = this.light
      ? this.light.flow(builder, 'v3', { cache: 'light' })
      : undefined;
    ...
    var emissive = this.emissive
      ? this.emissive.flow(builder, 'c', { slot: 'emissive' })
      : undefined;
    ...
    var clearcoatEnv =
      useClearcoat && environment
        ? this.environment.flow(builder, 'c', {
            cache: 'clearcoat',
    ...
    output.push(
      'material.diffuseColor = ' +
        (light ? 'vec3( 1.0 )' : 'diffuseColor * ( 1.0 - metalnessFactor )') +
        ';',
    ...
    if (clearcoatRoughness) {
      output.push(
        ...
        'material.clearcoatRoughness = max( ' +
          clearcoatRoughness.result +
          ', 0.0525 );',
    ...
    if (reflectivity) {
      output.push(
        reflectivity.code,
        'material.specularColor = mix( vec3( MAXIMUM_SPECULAR_COEFFICIENT * pow2( ' +
          reflectivity.result +
