...
var Ocean = function(renderer, camera, scene, options) {
  ...
  this.oceanColor = optionalParameter(
    options.OCEAN_COLOR,
    new Vector3(0.004, 0.016, 0.047),
  );
  this.skyColor = optionalParameter(
    options.SKY_COLOR,
    new Vector3(3.2, 9.6, 12.8),
  );
  ...
  var renderTargetType = optionalParameter(options.USE_HALF_FLOAT, false)
    ? HalfFloatType
    : FloatType;
  ...
  this.initialSpectrumFramebuffer = new WebGLRenderTarget(
    this.resolution,
    this.resolution,
    NearestRepeatParams,
  );
  this.spectrumFramebuffer = new WebGLRenderTarget(
    this.resolution,
    this.resolution,
    NearestClampParams,
  );
  this.pingPhaseFramebuffer = new WebGLRenderTarget(
    this.resolution,
    this.resolution,
    NearestClampParams,
  );
  this.pongPhaseFramebuffer = new WebGLRenderTarget(
    this.resolution,
    this.resolution,
    NearestClampParams,
  );
  this.pingTransformFramebuffer = new WebGLRenderTarget(
    this.resolution,
    this.resolution,
    NearestClampParams,
  );
  this.pongTransformFramebuffer = new WebGLRenderTarget(
    this.resolution,
    this.resolution,
    NearestClampParams,
  );
  this.displacementMapFramebuffer = new WebGLRenderTarget(
    this.resolution,
    this.resolution,
    LinearClampParams,
  );
  this.normalMapFramebuffer = new WebGLRenderTarget(
    this.resolution,
    this.resolution,
    LinearClampParams,
  );
  ...
  var oceanHorizontalUniforms = UniformsUtils.clone(
    oceanHorizontalShader.uniforms,
  );
  ...
  var initialSpectrumUniforms = UniformsUtils.clone(
    initialSpectrumShader.uniforms,
  );
...
Ocean.prototype.generateSeedPhaseTexture = function() {
  ...
  this.pingPhase = true;
  var phaseArray = new window.Float32Array(
  ...
  this.pingPhaseTexture = new DataTexture(
    phaseArray,
    this.resolution,
    this.resolution,
    RGBAFormat,
...
Ocean.prototype.renderSpectrumFFT = function() {
  ...
  for (var i = 0; i < iterations; i++) {
    if (i === 0) {
      ...
      this.materialOceanHorizontal.uniforms.u_subtransformSize.value = Math.pow(
        2,
        (i % iterations) + 1,
      );
  ...
  for (var i = iterations; i < iterations * 2; i++) {
    if (i === iterations * 2 - 1) {
      this.materialOceanVertical.uniforms.u_input.value =
        iterations % 2 === 0
          ? this.pingTransformFramebuffer.texture
          : this.pongTransformFramebuffer.texture;
      this.materialOceanVertical.uniforms.u_subtransformSize.value = Math.pow(
        2,
        (i % iterations) + 1,
      );
