...
CinematicCamera.prototype.setLens = function(
  focalLength,
  filmGauge,
  fNumber,
  coc,
...
CinematicCamera.prototype.focusAt = function(focusDistance) {
  ...
  this.nearPoint =
    (this.hyperFocal * this.focus) /
    (this.hyperFocal + (this.focus - focalLength));
...
CinematicCamera.prototype.initPostProcessing = function() {
  if (this.postprocessing.enabled) {
    ...
    this.postprocessing.rtTextureColor = new WebGLRenderTarget(
      window.innerWidth,
      window.innerHeight,
      pars,
    );
    ...
    this.postprocessing.bokeh_uniforms = UniformsUtils.clone(
      bokeh_shader.uniforms,
    );

    this.postprocessing.bokeh_uniforms[
      'tColor'
    ].value = this.postprocessing.rtTextureColor.texture;
    this.postprocessing.bokeh_uniforms[
      'tDepth'
