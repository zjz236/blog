...
export default QUnit.module('Polyfills', () => {
  ...
  QUnit.test('Number.EPSILON', assert => {
    assert.equal(
      Number.EPSILON,
      Math.pow(2, -52),
      'Number.EPSILON should be equal to 2 to the power of -52.',
    );
  });
  ...
  QUnit.test('Number.isInteger', assert => {
    ...
    assert.ok(
      Number.isInteger(99999999999999999999999),
      '99999999999999999999999 is an integer.',
    );
  ...
  QUnit.test('Math.sign', assert => {
    ...
    assert.ok(
      isNaN(Math.sign(new Vector3())),
      'If x is NaN<object>, the result is NaN.',
    );
    assert.ok(isNaN(Math.sign()), 'If x is NaN<undefined>, the result is NaN.');
    assert.ok(
      isNaN(Math.sign('--3')),
      "If x is NaN<'--3'>, the result is NaN.",
    );
    ...
    assert.ok(
      ...
      'If x is negative<-Infinity> and not -0, the result is -1.',
    );
    assert.ok(
      Math.sign('-3') === -1,
      "If x is negative<'-3'> and not -0, the result is -1.",
    );
    assert.ok(
      Math.sign('-1e-10') === -1,
      "If x is negative<'-1e-10'> and not -0, the result is -1.",
    );
    assert.ok(
      Math.sign(+Infinity) === +1,
      'If x is positive<+Infinity> and not +0, the result is +1.',
    );
    assert.ok(
      Math.sign('+3') === +1,
      "If x is positive<'+3'> and not +0, the result is +1.",
    );
  ...
  QUnit.test("'name' in Function.prototype", assert => {
    ...
    assert.equal(
      ...
      '',
      'Name on the prototype of Function should be declared.',
    );
    assert.equal(
      test.name,
      'test',
      "Name of function 'test' should be 'test'.",
    );
  });
  ...
  QUnit.test('Object.assign', assert => {
    ...
    assert.equal(
      ...
      4,
      'The length should be 4 in the final object.',
    );
    ...
    assert.equal(
      Object.keys(result).length,
      7,
      'The length should be 7 in the final object.',
    );
    ...
    assert.equal(
      result,
      target,
      'null and undefined should be ignored, result should be original object.',
    );
    ...
    assert.equal(
      result,
      target,
      'Numbers, booleans, and symbols cannot have wrappers with own enumerable properties.',
    );
    ...
    assert.equal(
      ...
      2,
      'The length property of the assign method should be 2.',
    );
