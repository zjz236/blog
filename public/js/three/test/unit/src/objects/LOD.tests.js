  ...
  QUnit.module('LOD', () => {
    ...
    QUnit.test('autoUpdate', assert => {
      ...
      assert.strictEqual(
        lod.autoUpdate,
        true,
        'LOD.autoUpdate is of type boolean and true by default.',
      );
    });
    ...
    QUnit.test('copy', assert => {
      ...
      assert.strictEqual(
        lod2.autoUpdate,
        false,
        'LOD.autoUpdate is correctly copied.',
      );
      assert.strictEqual(
        lod2.levels.length,
        3,
        'LOD.levels has the correct length after the copy.',
      );
    });
    QUnit.test('addLevel', assert => {
      ...
      assert.strictEqual(
        lod.levels.length,
        3,
        'LOD.levels has the correct length.',
      );
      assert.deepEqual(
        lod.levels[0],
        { distance: 5, object: high },
        'First entry correct.',
      );
      assert.deepEqual(
        lod.levels[1],
        { distance: 25, object: mid },
        'Second entry correct.',
      );
      assert.deepEqual(
        lod.levels[2],
        { distance: 50, object: low },
        'Third entry correct.',
      );
    });
    QUnit.test('getObjectForDistance', assert => {
      ...
      assert.strictEqual(
        lod.getObjectForDistance(25),
        mid,
        'Returns the mid resolution object.',
      );
      assert.strictEqual(
        lod.getObjectForDistance(50),
        low,
        'Returns the low resolution object.',
      );
      assert.strictEqual(
        lod.getObjectForDistance(60),
        low,
        'Returns the low resolution object.',
      );
    });
