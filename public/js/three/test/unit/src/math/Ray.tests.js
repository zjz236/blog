  ...
  QUnit.module('Ray', () => {
    ...
    QUnit.test('recast/clone', assert => {
      ...
      assert.ok(
        b
          .recast(-1)
          .equals(new Ray(new Vector3(1, 1, 0), new Vector3(0, 0, 1))),
        'Passed!',
      );

      var c = a.clone();
      assert.ok(
        c.recast(1).equals(new Ray(new Vector3(1, 1, 2), new Vector3(0, 0, 1))),
        'Passed!',
      );
    ...
    QUnit.test('intersectPlane', assert => {
      ...
      var e = new Plane().setFromNormalAndCoplanarPoint(
        new Vector3(1, 0, 0),
        one3,
      );
      ...
      var f = new Plane().setFromNormalAndCoplanarPoint(
        new Vector3(1, 0, 0),
        zero3,
      );
    ...
    QUnit.test('intersectTriangle', assert => {
      ...
      assert.ok(
        Math.abs(point.x - 2 / 3) <= eps,
        'Successful intersection: check x',
      );
      assert.ok(
        Math.abs(point.y - 2 / 3) <= eps,
        'Successful intersection: check y',
      );
      assert.ok(
        Math.abs(point.z - 2 / 3) <= eps,
