...
export default QUnit.module('Maths', () => {
  QUnit.module('Math', () => {
    ...
    QUnit.test('clamp', assert => {
      assert.strictEqual(
        MathUtils.clamp(0.5, 0, 1),
        0.5,
        'Value already within limits',
      );
      assert.strictEqual(
        MathUtils.clamp(0, 0, 1),
        0,
    ...
    QUnit.test('euclideanModulo', assert => {
      assert.ok(
        isNaN(MathUtils.euclideanModulo(6, 0)),
        'Division by zero returns NaN',
      );
      assert.strictEqual(
        MathUtils.euclideanModulo(6, 1),
        0,
        'Divison by trivial divisor',
      );
      assert.strictEqual(
        MathUtils.euclideanModulo(6, 2),
        0,
        'Divison by non-trivial divisor',
      );
      assert.strictEqual(
        MathUtils.euclideanModulo(6, 5),
        1,
        'Divison by itself - 1',
      );
      assert.strictEqual(
        MathUtils.euclideanModulo(6, 6),
        0,
        'Divison by itself',
      );
      assert.strictEqual(
        MathUtils.euclideanModulo(6, 7),
        6,
        'Divison by itself + 1',
      );
    });
    ...
    QUnit.test('mapLinear', assert => {
      assert.strictEqual(
        MathUtils.mapLinear(0.5, 0, 1, 0, 10),
        5,
        'Value within range',
      );
      assert.strictEqual(
        MathUtils.mapLinear(0.0, 0, 1, 0, 10),
        0,
        'Value equal to lower boundary',
      );
      assert.strictEqual(
        MathUtils.mapLinear(1.0, 0, 1, 0, 10),
        10,
    ...
    QUnit.test('lerp', assert => {
      assert.strictEqual(
        MathUtils.lerp(1, 2, 0),
        1,
        'Value equal to lower boundary',
      );
      assert.strictEqual(
        MathUtils.lerp(1, 2, 1),
        2,
        'Value equal to upper boundary',
      );
    ...
    QUnit.test('smoothstep', assert => {
      assert.strictEqual(
        MathUtils.smoothstep(-1, 0, 2),
        0,
        'Value lower than minimum',
      );
      assert.strictEqual(
        MathUtils.smoothstep(0, 0, 2),
        0,
        'Value equal to minimum',
      );
      assert.strictEqual(
        MathUtils.smoothstep(0.5, 0, 2),
        0.15625,
        'Value within limits',
      );
      assert.strictEqual(
      ...
      assert.strictEqual(
        MathUtils.smoothstep(1.5, 0, 2),
        0.84375,
        'Value within limits',
      );
      assert.strictEqual(
        MathUtils.smoothstep(2, 0, 2),
        1,
        'Value equal to maximum',
      );
      assert.strictEqual(
        MathUtils.smoothstep(3, 0, 2),
        1,
        'Value highter than maximum',
      );
    });

    QUnit.test('smootherstep', assert => {
      assert.strictEqual(
        MathUtils.smootherstep(-1, 0, 2),
        0,
        'Value lower than minimum',
      );
      assert.strictEqual(
        MathUtils.smootherstep(0, 0, 2),
        0,
        'Value equal to minimum',
      );
      assert.strictEqual(
        MathUtils.smootherstep(0.5, 0, 2),
        0.103515625,
        'Value within limits',
      );
      assert.strictEqual(
      ...
      assert.strictEqual(
        MathUtils.smootherstep(1.5, 0, 2),
        0.896484375,
        'Value within limits',
      );
      assert.strictEqual(
        MathUtils.smootherstep(2, 0, 2),
        1,
        'Value equal to maximum',
      );
      assert.strictEqual(
        MathUtils.smootherstep(3, 0, 2),
        1,
        'Value highter than maximum',
      );
    });
    ...
    QUnit.test('ceilPowerOfTwo', assert => {
      assert.strictEqual(
        MathUtils.ceilPowerOfTwo(1),
        1,
        'Closest higher PoT to 1 is 1',
      );
      assert.strictEqual(
        MathUtils.ceilPowerOfTwo(3),
        4,
        'Closest higher PoT to 3 is 4',
      );
      assert.strictEqual(
        MathUtils.ceilPowerOfTwo(4),
        4,
        'Closest higher PoT to 4 is 4',
      );
    });

    QUnit.test('floorPowerOfTwo', assert => {
      assert.strictEqual(
        MathUtils.floorPowerOfTwo(1),
        1,
        'Closest lower PoT to 1 is 1',
      );
      assert.strictEqual(
        MathUtils.floorPowerOfTwo(3),
        2,
        'Closest lower PoT to 3 is 2',
      );
      assert.strictEqual(
        MathUtils.floorPowerOfTwo(4),
        4,
        'Closest lower PoT to 4 is 4',
      );
    });
  });
});
