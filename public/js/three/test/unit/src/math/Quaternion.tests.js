...
function slerpTestSkeleton(doSlerp, maxError, assert) {
  ...
  assert.ok(
    Math.abs(result.dotA - result.dotB) <= Number.EPSILON,
    'Symmetry at 0.5',
  );
...
export default QUnit.module('Maths', () => {
  QUnit.module('Quaternion', () => {
    ...
    QUnit.test('setFromAxisAngle', assert => {
      ...
      var b2 = new Quaternion().setFromAxisAngle(
        new Vector3(1, 0, 0),
        -Math.PI,
      );
    ...
    QUnit.test('setFromEuler/setFromRotationMatrix', assert => {
      ...
      for (var i = 0; i < orders.length; i++) {
        var q = new Quaternion().setFromEuler(
          changeEulerOrder(eulerAngles, orders[i]),
        );
        var m = new Matrix4().makeRotationFromEuler(
          changeEulerOrder(eulerAngles, orders[i]),
        );
    ...
    QUnit.test('setFromRotationMatrix', assert => {
      ...
      assert.ok(
        Math.abs(a.x - expected.x) <= eps,
        'm11 > m22 && m11 > m33: check x',
      );
      assert.ok(
        Math.abs(a.y - expected.y) <= eps,
        'm11 > m22 && m11 > m33: check y',
      );
      assert.ok(
        Math.abs(a.z - expected.z) <= eps,
        'm11 > m22 && m11 > m33: check z',
      );
      assert.ok(
        Math.abs(a.w - expected.w) <= eps,
        'm11 > m22 && m11 > m33: check w',
      );
    ...
    QUnit.test('multiplyQuaternions/multiply', assert => {
      ...
      var q1 = new Quaternion().setFromEuler(
        changeEulerOrder(angles[0], 'XYZ'),
      );
      var q2 = new Quaternion().setFromEuler(
        changeEulerOrder(angles[1], 'XYZ'),
      );
      var q3 = new Quaternion().setFromEuler(
        changeEulerOrder(angles[2], 'XYZ'),
      );

      var q = new Quaternion().multiplyQuaternions(q1, q2).multiply(q3);

      var m1 = new Matrix4().makeRotationFromEuler(
        changeEulerOrder(angles[0], 'XYZ'),
      );
      var m2 = new Matrix4().makeRotationFromEuler(
        changeEulerOrder(angles[1], 'XYZ'),
      );
      var m3 = new Matrix4().makeRotationFromEuler(
        changeEulerOrder(angles[2], 'XYZ'),
      );
    ...
    QUnit.test('fromBufferAttribute', assert => {
      ...
      var attribute = new BufferAttribute(
        new Float32Array([0, 0, 0, 1, 0.7, 0, 0, 0.7, 0, 0.7, 0, 0.7]),
        4,
      );
    ...
    QUnit.test('multiplyVector3', assert => {
      ...
      for (var i = 0; i < orders.length; i++) {
        for (var j = 0; j < angles.length; j++) {
          var q = new Quaternion().setFromEuler(
            changeEulerOrder(angles[j], orders[i]),
          );
          var m = new Matrix4().makeRotationFromEuler(
            changeEulerOrder(angles[j], orders[i]),
          );
