...
export default QUnit.module('Maths', () => {
  QUnit.module('Spherical', () => {
    ...
    QUnit.test('setFromVector3', assert => {
      ...
      a.setFromVector3(c);
      assert.ok(
        Math.abs(a.radius - expected.radius) <= eps,
        'Normal vector: check radius',
      );
      assert.ok(
        Math.abs(a.phi - expected.phi) <= eps,
        'Normal vector: check phi',
      );
      assert.ok(
        Math.abs(a.theta - expected.theta) <= eps,
        'Normal vector: check theta',
      );
    });
  });
});
