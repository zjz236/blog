  ...
  QUnit.module('AnimationObjectGroup', () => {
    ...
    QUnit.test('smoke test', assert => {
      var expect = function expect(
        ...
        assert.ok(
          group.nCachedObjects_ === cached,
          QUnit.testIndex + ' cache size',
        );
        assert.ok(
          bindings.length - group.nCachedObjects_ === roots.length,
          QUnit.testIndex + ' object count',
        );
      };
      ...
      assert.ok(
        groupB instanceof AnimationObjectGroup,
