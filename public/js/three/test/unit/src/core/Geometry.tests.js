  ...
  QUnit.module('Geometry', () => {
    ...
    QUnit.test('rotateX', assert => {
      ...
      var v0 = geometry.vertices[0],
        ...
        v2 = geometry.vertices[2];
      assert.ok(
        v0.x === -0.5 && v0.y === 0 && v0.z === 0,
        'first vertex was rotated',
      );
      assert.ok(
        v1.x === 0.5 && v1.y === 0 && v1.z === 0,
        'second vertex was rotated',
      );
      assert.ok(
        v2.x === 0 && v2.y < Number.EPSILON && v2.z === 1,
        'third vertex was rotated',
      );
    });
    ...
    QUnit.test('rotateY', assert => {
      ...
      var v0 = geometry.vertices[0],
        ...
        v2 = geometry.vertices[2];
      assert.ok(
        v0.x === 0.5 && v0.y === 0 && v0.z < Number.EPSILON,
        'first vertex was rotated',
      );
      assert.ok(
        v1.x === -0.5 && v1.y === 0 && v1.z < Number.EPSILON,
        'second vertex was rotated',
      );
      assert.ok(
    ...
    QUnit.test('rotateZ', assert => {
      ...
      var v0 = geometry.vertices[0],
        ...
        v2 = geometry.vertices[2];
      assert.ok(
        v0.x < Number.EPSILON && v0.y === 0.5 && v0.z === 0,
        'first vertex was rotated',
      );
      assert.ok(
        v1.x < Number.EPSILON && v1.y === -0.5 && v1.z === 0,
        'second vertex was rotated',
      );
      assert.ok(
        v2.x === 1 && v2.y < Number.EPSILON && v2.z === 0,
        'third vertex was rotated',
      );
    });
    ...
    QUnit.test('fromBufferGeometry', assert => {
      ...
      bufferGeometry.setAttribute(
        'color',
        new BufferAttribute(
          new Float32Array([0, 0, 0, 0.5, 0.5, 0.5, 1, 1, 1]),
          3,
        ),
      );
      bufferGeometry.setAttribute(
        'normal',
        new BufferAttribute(new Float32Array([0, 1, 0, 1, 0, 1, 1, 1, 0]), 3),
      );
      bufferGeometry.setAttribute(
        'uv',
        new BufferAttribute(new Float32Array([0, 0, 0, 1, 1, 1]), 2),
      );
      bufferGeometry.setAttribute(
        'uv2',
        new BufferAttribute(new Float32Array([0, 0, 0, 1, 1, 1]), 2),
    ...
    QUnit.test('computeBoundingBox', assert => {
      ...
      assert.strictEqual(
        a.boundingBox.isEmpty(),
        false,
        "Bounding box isn't empty",
      );
    ...
    QUnit.test('computeBoundingSphere', assert => {
      ...
      assert.strictEqual(
        allIn,
        true,
        'All vertices are inside the bounding sphere',
      );
    });
    ...
    QUnit.test('mergeVertices', assert => {
      ...
      assert.strictEqual(
        removed,
        16,
        'Removed the expected number of vertices',
      );
