...
export default QUnit.module('Core', () => {
  QUnit.module('BufferAttribute', () => {
    ...
    QUnit.test('copy', assert => {
      ...
      assert.ok(
        attr.array.length === attrCopy.array.length,
        'array length is equal',
      );
      assert.ok(
        attr.version === 1 && attrCopy.version === 0,
    ...
    QUnit.test('copyAt', assert => {
      ...
      assert.ok(
        ...
        'chunck copied to correct place',
      );
      assert.ok(
    ...
    QUnit.test('copyColorsArray', assert => {
      ...
      assert.ok(
        i[0] === 0 && i[1] === 0.5 && i[2] === 1,
        'first color was copied correctly',
      );
      assert.ok(
        i[3] === 0.25 && i[4] === 1 && i[5] === 0,
        'second color was copied correctly',
      );
    });
    ...
    QUnit.test('copyVector3sArray', assert => {
      ...
      assert.ok(
        i[0] === 1 && i[1] === 2 && i[2] === 3,
        'first vector was copied correctly',
      );
      assert.ok(
        i[3] === 10 && i[4] === 20 && i[5] === 30,
        'second vector was copied correctly',
      );
    });
    ...
    QUnit.test('onUpload', assert => {
      ...
      assert.strictEqual(
        a.onUploadCallback,
        func,
        'Check callback was set properly',
      );
    });
    ...
    QUnit.test('count', assert => {
      assert.ok(
        new BufferAttribute(new Float32Array([1, 2, 3, 4, 5, 6]), 3).count ===
          2,
