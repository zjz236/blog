...
export default QUnit.module('Core', () => {
  QUnit.module('EventDispatcher', () => {
    ...
    QUnit.test('addEventListener', assert => {
      ...
      assert.ok(
        eventDispatcher._listeners.anyType[0] === listener,
        'listener with unknown type was added',
      );
      ...
      assert.ok(
        eventDispatcher._listeners.anyType.length === 1,
        "can't add one listener twice to same type",
      );
      assert.ok(
        eventDispatcher._listeners.anyType[0] === listener,
    ...
    QUnit.test('hasEventListener', assert => {
      ...
      assert.ok(
        eventDispatcher.hasEventListener('anyType', listener),
        'listener was found',
      );
      assert.ok(
        !eventDispatcher.hasEventListener('anotherType', listener),
    ...
    QUnit.test('removeEventListener', assert => {
      ...
      assert.ok(
        eventDispatcher._listeners === undefined,
      ...
      assert.ok(
        eventDispatcher._listeners.anyType.length === 0,
        'listener was deleted',
      );

      eventDispatcher.removeEventListener('unknownType', listener);
      assert.ok(
        eventDispatcher._listeners.unknownType === undefined,
      ...
      assert.ok(
        eventDispatcher._listeners.anyType.length === 0,
        'undefined listeners are ignored',
      );
    });
