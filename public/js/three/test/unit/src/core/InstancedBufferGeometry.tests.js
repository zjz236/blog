...
export default QUnit.module('Core', () => {
  QUnit.module('InstancedBufferGeometry', () => {
    ...
    QUnit.test('copy', assert => {
      ...
      assert.ok(
        copiedInstance instanceof InstancedBufferGeometry,
        'the clone has the correct type',
      );

      assert.equal(copiedInstance.index, indexMock, 'index was copied');
      assert.equal(
        copiedInstance.index.callCount,
        1,
        'index.clone was called once',
      );

      assert.ok(
        copiedInstance.attributes['defaultAttribute1'] instanceof
          BufferAttribute,
        'attribute was created',
      );
      assert.deepEqual(
        copiedInstance.attributes['defaultAttribute1'].array,
        defaultAttribute1.array,
        'attribute was copied',
      );
      assert.deepEqual(
        copiedInstance.attributes['defaultAttribute2'].array,
        defaultAttribute2.array,
        'attribute was copied',
      );
      ...
      assert.equal(
        copiedInstance.groups[0].materialIndex,
        instanceMock1,
        'group was copied',
      );
      ...
      assert.equal(
        copiedInstance.groups[1].materialIndex,
        instanceMock2,
        'group was copied',
      );
    });
