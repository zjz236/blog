...
function checkRayDirectionAgainstReferenceVector(
  rayDirection,
  refVector,
  assert,
) {
...
export default QUnit.module('Core', () => {
  QUnit.module('Raycaster', () => {
    ...
    QUnit.test('setFromCamera (Perspective)', assert => {
      ...
      assert.ok(
        rayDirection.x === 0 && rayDirection.y === 0 && rayDirection.z === -1,
      ...
      for (var x = -1; x <= 1; x += step) {
        for (var y = -1; y <= 1; y += step) {
          ...
          checkRayDirectionAgainstReferenceVector(
            rayDirection,
            refVector,
            assert,
          );
    ...
    QUnit.test('intersectObject', assert => {
      ...
      assert.ok(
        raycaster.intersectObject(objectsToCheck[0]).length === 1,
        'no recursive search should lead to one hit',
      );

      assert.ok(
        raycaster.intersectObject(objectsToCheck[0], true).length === 3,
      ...
      for (var i = 0; i < intersections.length - 1; i++) {
        assert.ok(
          intersections[i].distance <= intersections[i + 1].distance,
    ...
    QUnit.test('intersectObjects', assert => {
      ...
      assert.ok(
        raycaster.intersectObjects(objectsToCheck).length === 1,
        'no recursive search should lead to one hit',
      );

      assert.ok(
        raycaster.intersectObjects(objectsToCheck, true).length === 3,
        'recursive search should lead to three hits',
      );
