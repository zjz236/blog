...
QUnit.module('utils', () => {
  QUnit.test('arrayMin', assert => {
    ...
    assert.equal(
      arrayMin([5]),
      5,
      'Single valued array should return the unique value as minimum',
    );
    ...
    assert.equal(
      arrayMin([-Infinity, 0, Infinity]),
      -Infinity,
      'The array [ - Infinity, 0, Infinity ] return -Infinity',
    );
  });
  ...
  QUnit.test('arrayMax', assert => {
    assert.equal(
      arrayMax([]),
      -Infinity,
      'Empty array return negative infinit',
    );
    assert.equal(
      arrayMax([5]),
      5,
      'Single valued array should return the unique value as maximum',
    );
