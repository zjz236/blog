  ...
  QUnit.module('Curves', () => {
    QUnit.module('QuadraticBezierCurve3', hooks => {
      ...
      QUnit.test('Simple curve', assert => {
        ...
        // symmetry
        var curveRev = new QuadraticBezierCurve3(curve.v2, curve.v1, curve.v0);
        ...
        assert.strictEqual(
          points.length,
          expectedPoints.length,
          'Reversed: Correct number of points',
        );
        assert.deepEqual(
          points,
      ...
      QUnit.test('getTangent/getTangentAt', assert => {
        ...
        expectedTangents.forEach(function(exp, i) {
          ...
          assert.numEqual(
            tangent.x,
            exp.x,
            'getTangentAt #' + i + ': x correct',
          );
          assert.numEqual(
            tangent.y,
            exp.y,
            'getTangentAt #' + i + ': y correct',
          );
        });
      });
        ...
        Object.keys(expected).forEach(function(group, i) {
          expected[group].forEach(function(vec, j) {
            assert.numEqual(
              frames[group][j].x,
              vec.x,
              'Frenet frames [' + i + ', ' + j + '].x correct',
            );
            assert.numEqual(
              frames[group][j].y,
              vec.y,
              'Frenet frames [' + i + ', ' + j + '].y correct',
            );
            assert.numEqual(
              frames[group][j].z,
              vec.z,
              'Frenet frames [' + i + ', ' + j + '].z correct',
            );
          });
        });
      });
