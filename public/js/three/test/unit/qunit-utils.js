...
QUnit.assert.equalKey = function(obj, ref, key) {
  ...
  var message =
    actual + ' should be equal to ' + expected + ' for key "' + key + '"';
  this.pushResult({
...
function checkGeometryJsonReading(json, geom) {
  ...
  var differingProp = getDifferingProp(
    output[geom.uuid],
    geom,
    excludedProperties,
  );
...
function checkLightCopyClone(assert, light) {
  ...
  QUnit.assert.notEqual(
    newLight.uuid,
    light.uuid,
    "Copied light's UUID differs from original",
  );
  QUnit.assert.notEqual(
    newLight.id,
    light.id,
    "Copied light's id differs from original",
  );
  ...
  QUnit.assert.notStrictEqual(
    ...
    light.color.getHex(),
    'Copied light is independent from original',
  );
  ...
  QUnit.assert.notEqual(
    clone.uuid,
    light.uuid,
    "Cloned light's UUID differs from original",
  );
  QUnit.assert.notEqual(
    clone.id,
    light.id,
    "Clone light's id differs from original",
  );
  ...
  QUnit.assert.notStrictEqual(
    clone.color.getHex(),
    light.color.getHex(),
    'Clone light is independent from original',
  );
