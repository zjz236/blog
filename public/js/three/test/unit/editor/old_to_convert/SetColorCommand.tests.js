...
QUnit.test('Test SetColorCommand (Undo and Redo)', function(assert) {
  ...
  assert.ok(
    pointLight.color.getHex() == colors[colors.length - 1],
    "OK, color has been set successfully (expected: '" +
      colors[colors.length - 1] +
      "', actual: '" +
      pointLight.color.getHex() +
      "')",
  );

  editor.undo();
  assert.ok(
    pointLight.color.getHex() == colors[colors.length - 2],
    "OK, color has been set successfully after undo (expected: '" +
  ...
  assert.ok(
    pointLight.color.getHex() == colors[colors.length - 1],
    "OK, color has been set successfully after redo (expected: '" +
      colors[colors.length - 1] +
      "', actual: '" +
      pointLight.color.getHex() +
      "')",
