...
QUnit.test('Test for SetMaterialValueCommand (Undo and Redo)', function(
  assert,
) {
  // setup scene
  ...
  testDataKeys.map(function(attributeName) {
    ...
    assert.ok(
      box.material[attributeName] == testData[attributeName][length - 1],
      'OK, ' +
        attributeName +
        " was set correctly to the last value (expected: '" +
        testData[attributeName][length - 1] +
        "', actual: '" +
        box.material[attributeName] +
        "')",
    );

    editor.undo();
    assert.ok(
      box.material[attributeName] == testData[attributeName][length - 2],
      'OK, ' +
        attributeName +
        " was set correctly to the second to the last value after undo (expected: '" +
        testData[attributeName][length - 2] +
        "', actual: '" +
        box.material[attributeName] +
        "')",
    );
    ...
    assert.ok(
      ...
      'OK, ' +
        attributeName +
        " was set correctly to the last value again after redo (expected: '" +
        testData[attributeName][length - 1] +
        "', actual: '" +
        box.material[attributeName] +
        "')",
