...
QUnit.test('Test AddScriptCommand (Undo and Redo)', function(assert) {
  ...
  assert.ok(
    editor.scene.children.length == 2,
  ...
  assert.ok(
    getScriptCount(editor) == scripts.length,
    'OK, correct number of scripts have been added',
  );

  for (var i = 0; i < objects.length; i++) {
    assert.ok(
      objects[i].uuid == scriptsKeys[i],
      'OK, script key #' + i + " matches the object's UUID",
    );
  }

  editor.undo();
  assert.ok(
    getScriptCount(editor) == scripts.length - 1,
    'OK, one script has been removed by undo',
  );

  editor.redo();
  assert.ok(
    getScriptCount(editor) == scripts.length,
    'OK, one script has been added again by redo',
  );

  for (var i = 0; i < scriptsKeys.length; i++) {
    assert.ok(
      editor.scripts[scriptsKeys[i]][0] == scripts[i],
      'OK, script #' + i + ' is still assigned correctly',
    );
  }
});
