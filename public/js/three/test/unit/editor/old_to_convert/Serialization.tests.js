...
QUnit.test('Test Serialization', function(assert) {
  ...
  var addScript = function() {
    ...
    var cmd = new AddScriptCommand(box, {
      name: 'test',
  ...
  var setGeometry = function() {
    ...
    var cmd = new SetGeometryCommand(
      box,
      getGeometry('BoxGeometry', boxGeometry),
    );
    cmd.updatable = false;
    editor.execute(cmd);
  ...
  function performTests() {
    ...
    for (var i = 0; i < setups.length; i++) {
      ...
      assert.ok(
        history == history2,
        'OK, forward serializing was successful for ' + name,
      );
    ...
    for (var i = 0; i < setups.length; i++) {
      ...
      assert.ok(
        history == history2,
        'OK, backward serializing was successful for ' + name,
      );
