...
QUnit.test('Test for SetMaterialCommand (Undo and Redo)', function(assert) {
  ...
  // initial test
  assert.ok(
    box.material.type == materialClasses[i],
    "OK, initial material type was set correctly (expected: '" +
      materialClasses[i] +
      "', actual: '" +
      box.material.type +
      "')",
  ...
  while (i > 0) {
    ...
    assert.ok(
      box.material.type == materialClasses[i],
      "OK, material type was set correctly after undo (expected: '" +
        materialClasses[i] +
        "', actual: '" +
        box.material.type +
        "')",
  ...
  while (i < materialClasses.length - 1) {
    ...
    assert.ok(
      box.material.type == materialClasses[i],
      "OK, material type was set correctly after redo (expected: '" +
        materialClasses[i] +
        "', actual: '" +
        box.material.type +
        "')",
