...
QUnit.test('Test MoveObjectCommand (Undo and Redo)', function(assert) {
  ...
  assert.ok(
    anakinSkywalker.parent.name == 'Scene',
    "OK, Anakin's parent is 'Scene' ",
  );
  assert.ok(
    lukeSkywalker.parent.name == 'Scene',
  ...
  assert.ok(
    lukeSkywalker.parent.name == anakinsName,
    "OK, Luke's parent is '" + anakinsName + "' ",
  );
  ...
  assert.ok(
    lukeSkywalker.parent.name == 'Scene',
    "OK, Luke's parent is 'Scene' again ",
  );
  ...
  assert.ok(
    lukeSkywalker.parent.name == anakinsName,
    "OK, Luke's parent is '" + anakinsName + "' again ",
  );
});
