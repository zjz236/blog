...
QUnit.test('Test SetGeometryValueCommand (Undo and Redo)', function(assert) {
  ...
  assert.ok(
    box.geometry.name == testData[1].name,
    'OK, box.geometry.name is correct after executes',
  );
  assert.ok(
    box.geometry.uuid == testData[1].uuid,
    'OK, box.geometry.uuid is correct after executes',
  );
  ...
  assert.ok(
    box.geometry.name == testData[0].name,
    'OK, box.geometry.name is correct after undos',
  );
  assert.ok(
    box.geometry.uuid == testData[0].uuid,
    'OK, box.geometry.uuid is correct after undos',
  );
