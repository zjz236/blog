...
QUnit.test('Test MultiCmdsCommand (Undo and Redo)', function(assert) {
  ...
  var boxGeometries = [
    getGeometry('BoxGeometry', boxGeometry1),
    getGeometry('BoxGeometry', boxGeometry2),
  ];
  ...
  assert.ok(
    box.geometry.parameters.widthSegments == 7,
    'OK, widthSegments has been modified accordingly after two multi executes (expected: 7, actual: ' +
      box.geometry.parameters.widthSegments +
      ')',
  );
  assert.ok(
    box.position.y == 5,
    'OK, y position has been modified accordingly after two multi executes (expected: 5, actual: ' +
      box.position.y +
      ')',
  );
  assert.ok(
    box.rotation.x == 0.4,
    'OK, x rotation has been modified accordingly after two multi executes (expected: 0.4, actual: ' +
      box.rotation.x +
      ') ',
  );
  assert.ok(
    box.scale.z == 1.6,
    'OK, z scale has been modified accordingly after two multi executes (expected: 1.6, actual: ' +
      box.scale.z +
      ')',
  );
  ...
  editor.undo();
  assert.ok(
    box.geometry.parameters.widthSegments == 2,
    'OK, widthSegments has been modified accordingly after undo (expected: 2, actual: ' +
      box.geometry.parameters.widthSegments +
      ')',
  );
  assert.ok(
    box.position.y == 2,
    'OK, y position has been modified accordingly after undo (expected: 2, actual: ' +
      box.position.y +
      ')',
  );
  assert.ok(
    box.rotation.x == 0.1,
    'OK, x rotation has been modified accordingly after undo (expected: 0.1, actual: ' +
      box.rotation.x +
      ')',
  );
  assert.ok(
    box.scale.z == 1.3,
    'OK, z scale has been modified accordingly after undo (expected: 1.3, actual: ' +
      box.scale.z +
      ')',
  );
