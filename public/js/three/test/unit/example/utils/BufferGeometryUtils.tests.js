...
export default QUnit.module('Utils', () => {
  QUnit.module('BufferGeometryUtils', () => {
    QUnit.test('mergeBufferAttributes - basic', assert => {
      ...
      var mergedAttr = BufferGeometryUtils.mergeBufferAttributes([
        attr1,
        attr2,
      ]);

      assert.smartEqual(
        Array.from(mergedAttr.array),
        [1, 2, 3, 4, 5, 6, 7, 8],
        'merges elements',
      );
    ...
    QUnit.test('mergeBufferGeometries - basic', assert => {
      ...
      geometry1.setAttribute(
        'position',
        new BufferAttribute(new Float32Array([1, 2, 3]), 1, false),
      );

      var geometry2 = new BufferGeometry();
      geometry2.setAttribute(
        'position',
        new BufferAttribute(new Float32Array([4, 5, 6]), 1, false),
      );
      ...
      assert.smartEqual(
        Array.from(mergedGeometry.attributes.position.array),
        [1, 2, 3, 4, 5, 6],
        'merges elements',
      );
      assert.equal(
        mergedGeometry.attributes.position.itemSize,
        1,
        'retains .itemSize',
      );
    });
    ...
    QUnit.test('mergeBufferGeometries - indexed', assert => {
      ...
      geometry1.setIndex(
        new BufferAttribute(new Uint16Array([0, 1, 2, 2, 1, 0]), 1, false),
      );
      ...
      geometry2.setIndex(
        new BufferAttribute(new Uint16Array([0, 1, 2]), 1, false),
      );
      ...
      assert.smartEqual(
        Array.from(mergedGeometry.index.array),
        [0, 1, 2, 2, 1, 0, 3, 4, 5],
        'merges indices',
      );
      assert.equal(
    ...
    QUnit.test('mergeBufferGeometries - morph targets', assert => {
      ...
      assert.smartEqual(
        Array.from(mergedGeometry.morphAttributes.position[0].array),
        [10, 20, 30, 40, 50, 60],
        'merges morph targets',
      );
      assert.smartEqual(
        Array.from(mergedGeometry.morphAttributes.position[1].array),
    ...
    QUnit.test('mergeBufferGeometries - invalid', assert => {
      ...
      assert.notOk(
        BufferGeometryUtils.mergeBufferGeometries([geometry1, geometry2]),
      );
    ...
    QUnit.test('toTrianglesDrawMode()', assert => {
      ...
      geometry1 = BufferGeometryUtils.toTrianglesDrawMode(
        geometry1,
        TriangleStripDrawMode,
      );

      assert.deepEqual(
        geometry1.index.array,
        new Uint16Array([0, 1, 2, 3, 2, 1, 2, 3, 4, 5, 4, 3]),
        'correct triangle indices from triangle strip',
      );
      ...
      geometry2 = BufferGeometryUtils.toTrianglesDrawMode(
        geometry2,
        TriangleFanDrawMode,
      );

      assert.deepEqual(
        geometry2.index.array,
        new Uint16Array([0, 1, 2, 0, 2, 3, 0, 3, 4, 0, 4, 5]),
        'correct triangle indices from triangle fan',
      );
    });
  });
});
