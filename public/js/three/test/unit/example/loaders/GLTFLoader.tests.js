...
export default QUnit.module('Loaders', () => {
  QUnit.module('GLTFLoader', () => {
    ...
    QUnit.test('parse - basic', assert => {
      ...
      var array = new Float32Array([-1, -1, -1, 1, 1, 1, 4, 4, 4]);
          ...
          loader.parse(
            ...
            function(gltf) {
              ...
              assert.equal(
                ...
                meshOut.material.color.getHex(),
                'loads color',
              );
              assert.smartEqual(
                attrsIn.position.array,
                attrsOut.position.array,
                'loads positions',
              );
              assert.equal(
                undefined,
        ...
        function(binary) {
          loader.parse(
            ...
            function(gltf) {
              ...
              assert.equal(
                'node1.position',
                clipOut.tracks[0].name,
                'track name',
              );
              assert.smartEqual(
                clip.tracks[0].times,
                clipOut.tracks[0].times,
                'track times',
              );
              assert.smartEqual(
                clip.tracks[0].values,
                clipOut.tracks[0].values,
                'track values',
              );
