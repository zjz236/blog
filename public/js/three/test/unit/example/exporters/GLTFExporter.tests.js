  ...
  QUnit.module('GLTFExporter', () => {
    ...
    QUnit.test('parse - basic', assert => {
      ...
      exporter.parse(box, function(gltf) {
        ...
        assert.ok(
          ...
          'mesh contains position data',
        );
        assert.ok(
          primitive.attributes.NORMAL !== undefined,
          'mesh contains normal data',
        );
    ...
    QUnit.test('parse - individual morph targets', assert => {
      ...
      var position = new BufferAttribute(
        new Float32Array([0, 0, 0, 0, 0, 1, 1, 0, 1]),
        3,
      );
      ...
      var valuesB = [0, 1, 0];
      var valuesC = [0, 1, 0];
      var trackA = new VectorKeyframeTrack(
        '.morphTargetInfluences[a]',
        timesA,
        valuesA,
        InterpolateLinear,
      );
      var trackB = new VectorKeyframeTrack(
        '.morphTargetInfluences[b]',
        timesB,
        valuesB,
        InterpolateLinear,
      );
      var trackC = new VectorKeyframeTrack(
        '.morphTargetInfluences[c]',
        timesC,
        valuesC,
        InterpolateLinear,
      );

      var clip = new AnimationClip('clip1', undefined, [
        trackA,
        trackB,
        trackC,
      ]);
    ...
    QUnit.test('utils - insertKeyframe', assert => {
      ...
      assert.smartEqual(
        Array.from(track.values),
        [0, 5, 0.5, 4.5, 1, 4, 2, 3, 3, 2, 4, 1, 5, 0],
        'insert - value (linear)',
      );
      ...
      assert.smartEqual(
        Array.from(track.times),
        [5, 10, 15, 16, 20, 25, 30],
        'insert - time (discrete)',
      );
      assert.smartEqual(
        Array.from(track.values),
        [0, 5, 1, 4, 2, 3, 2, 3, 3, 2, 4, 1, 5, 0],
        'insert - value (discrete)',
      );
    ...
    QUnit.test('utils - mergeMorphTargetTracks', assert => {
      ...
      for (var i = 0; i < track.values.length; i++) {
        assert.numEqual(
          track.values[i],
          expectedValues[i],
          'all values are merged or interpolated - ' + i,
        );
      }
    });
  });
