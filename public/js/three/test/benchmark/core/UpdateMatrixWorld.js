(function() {
  ...
  var rootA = generateSceneGraph(
    new THREE.Object3D(),
    100,
    1,
    createLocallyOffsetChild,
  );
  var rootB = generateSceneGraph(
    new THREE.Object3D(),
    3,
    10,
    createLocallyOffsetChild,
  );
  var rootC = generateSceneGraph(
    new THREE.Object3D(),
    9,
    3,
    createLocallyOffsetChild,
  ...
  s.add(
    'Update graph depth=100, breadth=1 (' + nodeCount(rootA) + ' nodes)',
    function() {
      rootA.updateMatrixWorld(true);
    },
  );
  s.add(
    'Update graph depth=3, breadth=10 (' + nodeCount(rootB) + ' nodes)',
    function() {
      rootB.updateMatrixWorld(true);
    },
  );
  s.add(
    'Update graph depth=9, breadth=3 (' + nodeCount(rootC) + ' nodes)',
    function() {
