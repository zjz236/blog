...
const pup = puppeteer
  ...
  .then(async browser => {
    ...
    page.on('console', msg =>
      msg.text().slice(0, 8) === 'Warning.' ? console.null(msg.text()) : {},
    );
    ...
    const files = fs
      ...
      .filter(f =>
        process.argv.length > 2
          ? exactList.includes(f)
          : !exceptionList.includes(f),
      );
    ...
    const beginId = isParallel
      ? Math.floor((parseInt(process.env.CI.slice(0, 1)) * files.length) / 4)
      : 0;
    const endId = isParallel
      ? Math.floor(
          ((parseInt(process.env.CI.slice(-1)) + 1) * files.length) / 4,
        )
      : files.length;
    ...
    for (let id = beginId; id < endId; ++id) {
      ...
      while (attemptId < maxAttemptId) {
        ...
        try {
          ...
          await page.evaluate(
            async (
              ...
              let resourcesSize = Math.min(
                1,
                (pageSize / 1024 / 1024 - pageSizeMinTax) / pageSizeMaxTax,
              );
              await new Promise(resolve =>
                setTimeout(
                  resolve,
                  networkTax * resourcesSize * attemptProgress,
                ),
              );
              ...
              await new Promise(function(resolve) {
                ...
                let waitingLoop = setInterval(function() {
                  let renderEcceded =
                    performance.wow() - renderStart >
                    renderTimeout * attemptProgress;
          ...
          if (++attemptId === maxAttemptId) {
            console.red(
              `WTF? 'Network timeout' is small for your machine. file: ${file} \n${e}`,
            );
            ...
            await new Promise(resolve =>
              setTimeout(resolve, networkTimeout * attemptProgress),
            );
          }
        }
        ...
        if (process.env.MAKE) {
          ...
          printImage(
            png.sync.read(
              fs.readFileSync(`./examples/screenshots/${file}.png`),
            ),
            console,
          );
          ...
          if (numFailedPixels < maxFailedPixels) {
            ...
            if (++attemptId === maxAttemptId) {
              ...
              console.red(
                `ERROR! Diff wrong in ${numFailedPixels.toFixed(
                  3,
                )} of pixels in file: ${file}`,
              );
              failedScreenshots.push(file);
              continue;
            } else {
    ...
    if (failedScreenshots.length) {
      if (failedScreenshots.length > 1) {
        console.red(
          'List of failed screenshots: ' + failedScreenshots.join(' '),
        );
      } else {
        console.red(
          `If you sure that all is right, try to run \`npm run make-screenshot ${failedScreenshots[0]}\``,
        );
      }

      console.red(
        `TEST FAILED! ${failedScreenshots.length} from ${endId -
          beginId} screenshots not pass.`,
      );
    } else if (!process.env.MAKE) {
      console.green(
        `TEST PASSED! ${endId - beginId} screenshots correctly rendered.`,
      );
    }
