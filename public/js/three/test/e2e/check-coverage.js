...
// screenshots
const S = fs
  .readdirSync('./examples/screenshots')
...
if (subES.length + subSE.length + subEF.length + subFE.length === 0) {
  console.green(
    'TEST PASSED! All examples is covered with screenshots and descriptions in files.js!',
  );
} else {
  if (subES.length > 0)
    console.red('Add screenshots for example(s): ' + subES.join(' '));
  if (subSE.length > 0)
    console.red('Remove unnecessary screenshot(s): ' + subSE.join(' '));
  if (subEF.length > 0)
    console.red(
      'Add description in file.js for example(s): ' + subEF.join(' '),
    );
  if (subFE.length > 0)
    console.red(
      'Remove description in file.js for example(s): ' + subFE.join(' '),
    );
